                        ...
                        reservations.map((res) => (
                            <div key={res.id} className={`glass-card flex flex-col md:flex-row justify-between items-center gap-6 ${res.status === 'cancelled' ? 'opacity-50 grayscale' : ''
                                ...
                                <div className="flex items-center gap-6 w-full md:w-auto">
                                    <div className="bg-indigo-600/20 p-4 rounded-2xl text-center min-w-[100px]">
                                        <p className="text-xs font-bold text-indigo-400 uppercase">{format(new Date(res.slot.date), 'EEE', { locale: hr })}</p>
